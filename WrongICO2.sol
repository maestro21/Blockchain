...
contract MyFirstERC20Coin {
    ...
    function _buy(address _from, uint _value) internal {	
        ...
        uint amount = _value / _calculatePrice();
        // Вызываем внутреннюю функцию перевода токенов
        _transfer(this, _from, amount);
    }
	
	// Считаем цену - удваиваем каждый день
	function _calculatePrice() internal view returns(uint) {
		return buyPrice * (2 **_getDays());
	}
	
	// Считаем количество дней с момента запуска
	function _getDays() internal view returns(uint) {
		return (now - startTime) / 60 / 60 / 24; 
	}
}